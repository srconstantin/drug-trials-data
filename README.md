...
Step 1: connect clinical studies of drugs from clinicaltrials.gov with drug targets. Assign a drug target to each drug. (or "NULL" for unknown/undefined targets).

Step 2: define a study as "successful" if that drug is FDA-approved for the disease/condition in the study (or, for studies on healthy subjects, if the drug is FDA-approved for any indication.) Classify the study as YES (if successful), NO (if unsuccessful), or NULL (if unknown).
