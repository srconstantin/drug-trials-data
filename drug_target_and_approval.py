...
dataset = pd.read_csv("/Users/sarahconstantin/Downloads/interventionalstudies.csv")
drug_studies = dataset[dataset['Interventions'].str.startswith('DRUG', na=False)]

drug_studies.to_csv("drug_studies.csv")

INPUT_CSV = "drug_studies.csv"  # Your filtered drug studies CSV
...
def get_target_with_thinking_and_search(client, drug: str, indication: str, use_web_search: bool = True) -> dict:
    """
    Query Claude API with extended thinking and optional web search.
    
    Returns dict with 'target' and 'thinking' keys.
    """
    prompt = f"""What is the target of the following drug, in the context of the following indication? This will typically be a molecule or gene that the drug interacts with as its mechanism of action. If there is no target or if there is not enough information to identify it, please say "NULL". Otherwise say the name of the target. Do not add commentary or explanation, just the name of the target.
...
def get_approval_with_thinking_and_search(client, drug: str, indication: str, use_web_search: bool = True) -> dict:
    ...
    return {
        "approval": response_text.strip(),
        "thinking": thinking_content
    }

def get_approval_simple(client, drug: str, indication: str) -> dict:
    """
    Fallback: Query Claude API without extended thinking or web search.
    Use this if you encounter issues with extended thinking.
    """
    ...
    return {
        "approval": response.content[0].text.strip(),
        "thinking": None
    }
...
def process_studies(
    ...
    """
    print(f"[{datetime.now()}] Starting drug target and approval extraction")
    ...
    for idx_str, (target, approval) in checkpoint["results"].items():
        idx = int(idx_str)
        if idx < len(df):
            df.at[idx, "Target"] = target
            df.at[idx, "Approval"] = approval
            ...
            for attempt in range(MAX_RETRIES):
                try:
                    if use_extended_thinking:
                        result1 = get_target_with_thinking_and_search(
                            client, 
                            str(drug),
                            str(indication), 
                            use_web_search=use_web_search
                        )
                        result2 = get_approval_with_thinking_and_search(
                            client, 
                            str(drug),
                            str(indication), 
                            use_web_search=use_web_search
                        ...
                        result1 = get_target_simple(client, str(drug), str(indication))
                        result2 = get_approval_simple(client, str(drug), str(indication))
                    
                    target = result1["target"]
                    approval = result2["approval"]
                    df.at[idx, "Target"] = target
                    df.at[idx, "Approval"] = approval
                    checkpoint["results"][str(idx)] = (target, approval)
                    ...
                    print(f"  [{idx}/{total_rows}] {nct}: {target[:50]}{'...' if len(target) > 50 else ''} {approval}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Extract drug targets and FDA approval status from clinical trial data")
